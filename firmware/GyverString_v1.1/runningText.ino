...
void drawLetter(int index, int letter, int offset, byte color) {
  int8_t start_pos = 0, finish_pos = LET_WIDTH;
  int letterColor;
  if (color == 1) letterColor = mHSV(byte(offset * 10), 255, 255);
  else if (color == 2) letterColor = mHSV(byte(index * 30), 255, 255);
  else letterColor = mHSV(thisColor, 255, 255);
